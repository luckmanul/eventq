...
public class QuestionResourceIntTest {
     ...
     */
    public static Question createEntity(final EntityManager em) {
        final Question question = new Question().title(DEFAULT_TITLE).description(DEFAULT_DESCRIPTION)
                .feedback(DEFAULT_FEEDBACK).createDate(DEFAULT_CREATE_DATE).publish(DEFAULT_PUBLISH);
        return question;
    }
    ...
    public void createQuestion() throws Exception {
        final int databaseSizeBeforeCreate = questionRepository.findAll().size();

        // Create the Question
        final QuestionDTO questionDTO = questionMapper.toDto(question);
        restQuestionMockMvc.perform(post("/api/questions").contentType(TestUtil.APPLICATION_JSON_UTF8)
                .content(TestUtil.convertObjectToJsonBytes(questionDTO))).andExpect(status().isCreated());

        // Validate the Question in the database
        final List<Question> questionList = questionRepository.findAll();
        assertThat(questionList).hasSize(databaseSizeBeforeCreate + 1);
        final Question testQuestion = questionList.get(questionList.size() - 1);
        ...
        assertThat(testQuestion.isPublish()).isEqualTo(DEFAULT_PUBLISH);
    }
    ...
    public void createQuestionWithExistingId() throws Exception {
        ...
        restQuestionMockMvc.perform(post("/api/questions").contentType(TestUtil.APPLICATION_JSON_UTF8)
                .content(TestUtil.convertObjectToJsonBytes(questionDTO))).andExpect(status().isBadRequest());
    ...
    public void getAllQuestions() throws Exception {
        ...
        restQuestionMockMvc.perform(get("/api/questions?sort=id,desc")).andExpect(status().isOk())
    ...
    public void getQuestion() throws Exception {
        ...
        // Get the question
        restQuestionMockMvc.perform(get("/api/questions/{id}", question.getId())).andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
    ...
    public void updateQuestion() throws Exception {
        // Initialize the database
        questionRepository.saveAndFlush(question);
        final int databaseSizeBeforeUpdate = questionRepository.findAll().size();

        // Update the question
        final Question updatedQuestion = questionRepository.findOne(question.getId());
        updatedQuestion.title(UPDATED_TITLE).description(UPDATED_DESCRIPTION).feedback(UPDATED_FEEDBACK)
                .createDate(UPDATED_CREATE_DATE).publish(UPDATED_PUBLISH);
        final QuestionDTO questionDTO = questionMapper.toDto(updatedQuestion);

        restQuestionMockMvc.perform(put("/api/questions").contentType(TestUtil.APPLICATION_JSON_UTF8)
                .content(TestUtil.convertObjectToJsonBytes(questionDTO))).andExpect(status().isOk());
        ...
        assertThat(testQuestion.isPublish()).isEqualTo(UPDATED_PUBLISH);
    }
    ...
    public void deleteQuestion() throws Exception {
        // Initialize the database
        questionRepository.saveAndFlush(question);
        final int databaseSizeBeforeDelete = questionRepository.findAll().size();

        // Get the question
        restQuestionMockMvc
                .perform(delete("/api/questions/{id}", question.getId()).accept(TestUtil.APPLICATION_JSON_UTF8))
